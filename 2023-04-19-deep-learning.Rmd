...
loss_one_set <- function(set.indices, model){
  torch::with_no_grad({#for efficiency, gradients not necessary.
    ...
    set.pred.class <- apply(set.pred.loss$pred, 1, which.max)
    is.error <- set.label.vec != set.pred.class
    rbind(
      data.table(variable="loss",value=as.numeric(set.pred.loss$loss)),
      data.table(variable="error.percent",value=100*mean(as.numeric(is.error))))
  })
}
...
n.hidden.units <- 100
(convolutional.network <- torch::nn_sequential(
  seq2flat, 
  torch::nn_linear(n.flat, n.hidden.units),
  torch::nn_relu(),
  torch::nn_linear(n.hidden.units, n.classes)))
count_params <- function(model){
  sum(sapply(model$parameters, length))
}
(conv.params <- count_params(convolutional.network))

new_fully_connected <- function(n.hidden.layers){
  units.per.layer <- c(n.features, rep(n.hidden.units,n.hidden.layers), n.classes)
  seq.args <- list()
  for(input.i in seq(1, length(units.per.layer)-1)){
    output.i <- input.i+1
    seq.args[[input.i]] <- torch::nn_linear(
